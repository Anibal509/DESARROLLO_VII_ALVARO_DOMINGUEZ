...
function crecimientoVentas($ventasMeses) {
    $primerMes = $ventasMeses[0];
    $ultimoMes = end($ventasMeses);
    if ($primerMes == 0) {
        return $ultimoMes > 0 ? 100 : 0; // Evitar división por cero
    }
...
echo "\nCrecimiento de ventas por producto y región (primer al último mes):\n";
...
foreach ($ventas as $region => $productos) {
    ...
    foreach ($productos as $producto => $ventasProducto) {
        ...
        // Identificar el producto y región con mayor crecimiento
...
echo "Producto con mayor crecimiento: $productoTopCrecimiento en $regionTopCrecimiento ({$mayorCrecimiento}%)\n";
